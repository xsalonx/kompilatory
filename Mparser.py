...
def p_instruction_1(p):
    ...
    p[0] = AST.Scope(p[2], lineno=p.lineno(1))
...
def p_instruction_2(p):
    """instruction : expr ';'
                   ...
                   | return_statement ';' """
    if p[1] == "break":
        p[0] = AST.BreakInstruction(lineno=p.lineno(1))
    elif p[1] == "continue":
        p[0] = AST.ContinueInstruction(lineno=p.lineno(1))
    else:
        p[0] = p[1]
...
def p_if_statement(p):
    ...
    p[0] = AST.IfStatement(p[3], p[5], p[7], lineno=p.lineno(1)) if len(p) == 8 else AST.IfStatement(p[3], p[5],
                                                                                                     lineno=p.lineno(1))
...
def p_for_loop(p):
    ...
    p[0] = AST.ForLoop(AST.Variable(p[2]), p[4], p[5], lineno=p.lineno(1))


def p_while_loop(p):
    """while_loop : WHILE '(' expr ')' instruction """
    p[0] = AST.WhileLoop(p[3], p[5], lineno=p.lineno(1))


def p_range(p):
    """range : expr ':' expr """
    p[0] = AST.Range(p[1], p[3], lineno=p.lineno(2))


def p_print_statement(p):
    """print_statement : PRINT printables """
    p[0] = AST.PrintStatement(p[2], lineno=p.lineno(1))
...
def p_return_statement_1(p):
    ...
    p[0] = AST.ReturnStatement(lineno=p.lineno(1))
